...
export async function reachesMainThroughOpenPrs(
	...
	for (let depth = 0; depth < 20; depth++) {
		...
		url.searchParams.set("head", `${owner}:${baseRef}`);
		...
		const candidates = (await response.json()) as ParentPullRequest[];
		if (!Array.isArray(candidates) || candidates.length === 100) return false;
