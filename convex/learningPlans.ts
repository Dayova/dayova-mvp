...
const publicDocument = (document: Doc<"learningPlanDocuments">): PublicDocument => ({
  id: document._id,
...
const publicSession = (session: Doc<"learningPlanSessions">): PublicSession => ({
  id: session._id,
  phase: session.phase,
  title: session.title,
...
export const registerUploadedDocument = action({
  ...
  handler: async (ctx, args): Promise<Id<"learningPlanDocuments">> => {
    const context: {
      ...
      accessKey: string;
    } = await ctx.runQuery(internal.learningPlans.getUploadRegistrationContext, {
