...
const resolveMediaType = (fileType: string, fileName: string) => {
  ...
  return extensionToMediaType[fileExtension(fileName)] ?? "application/octet-stream";
};
...
const extractTextFromBytes = async (
  ...
  if (fileType.startsWith("text/") || plainTextExtensions.has(extension)) {
    return compactText(new TextDecoder("utf-8").decode(fileBuffer), MAX_EXTRACTED_TEXT_CHARS);
  }
...
const buildModelInputFromDocuments = async (
  ...
  return {
    fileParts,
    sourceContext: compactText(textSections.join("\n\n---\n\n"), MAX_PROMPT_CONTEXT_CHARS),
  };
};
...
const buildDateFromOffset = (examDateKey: string, dayOffsetBeforeExam: number) => {
