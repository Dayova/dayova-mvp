...
- Only completed sessions update Analyse. The overview lists every topic in the confirmed exam scope, including topics without answer evidence. Opening a topic shows a short current-knowledge summary and every related question as a horizontally paged card with only the learner's latest answer and its precise review; earlier attempts, ideal answers, the separate evidence-dimension profile, and visible paging instructions or counters stay hidden.
