...
- Only completed sessions update Analyse. The overview lists every topic in the confirmed exam scope, including topics without answer evidence. Opening a topic shows a clear exam context, the topic title with its readiness, a neutral learning-goal description, a primary action that continues with the plan's recommended session, and every related question as a horizontally paged card. Each answer card initially shows only the question; activating it flips the card by 180 degrees to reveal the learner's latest answer and an evaluation containing both a precise review and the correct answer, and activating it again returns to the question. The flip keeps a stable card size, exposes its state and complete content accessibly, and becomes an immediate state change when reduced motion is enabled. The detail header does not repeat topic priority or decorate the learning goal as a separate colored/icon card. Detailed diagnosis stays in the answer review; earlier attempts, separate ideal-answer cards or sections, separate knowledge-summary or evidence-dimension cards, control-check explanations, and visible paging instructions or counters stay hidden. When no recommended session remains, the primary action opens the learning plan instead.
- Answer feedback inside a session contains the review of the learner's answer and the ideal answer. Analyse topic pages integrate the correct answer into the evaluation instead of showing it as a separate ideal-answer section, and do not show separate strengths and weaknesses cards.
