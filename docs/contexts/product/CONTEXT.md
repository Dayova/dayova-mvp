...
- Only completed sessions update Analyse. The overview lists every topic in the confirmed exam scope, including topics without answer evidence. Opening a topic shows a clear exam context, the topic title with its readiness, a neutral learning-goal description, a primary action that continues with the plan's recommended session, and every related question as a horizontally paged card with only the learner's latest answer and its precise review. The detail header does not repeat topic priority or decorate the learning goal as a separate colored/icon card. Detailed diagnosis stays in the answer review; earlier attempts, ideal answers, separate knowledge-summary or evidence-dimension cards, control-check explanations, and visible paging instructions or counters stay hidden. When no recommended session remains, the primary action opens the learning plan instead.
