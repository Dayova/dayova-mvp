...
- Manual `Lernzeit` setup is not part of onboarding or the critical exam-to-plan path. When no saved availability exists, create grade-aware system defaults (grades 5–8 until 20:00, grades 9–10 until 22:00, grade 11+ until midnight), label them as Dayova proposals, and keep both the first learning action and later confirmation non-blocking. Planned times are guidelines and never prevent an earlier or later start. After the `Wissenscheck`, offer one dismissible learning-path reminder. After a stable pattern across at least five completed sessions, Dayova may suggest changed days and times, but never applies them without consent. Changing availability reschedules only future sessions and preserves completed progress. See [DAY-417](https://linear.app/dayova/issue/DAY-417/remove-manual-learning-times-from-the-critical-path-and-generate-defaults).
