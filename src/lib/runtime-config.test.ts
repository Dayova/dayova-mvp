...
describe("getMissingPublicRuntimeConfig", () => {
	...
	it("reads configured public app envs from the process environment", () => {
		...
		try {
			expect(readPublicRuntimeConfig()).toEqual({
				...
				EXPO_PUBLIC_NATIVE_STORE_PURCHASES_ENABLED: undefined,
	...
	it("requires store keys and app-specific legal URLs for releases", () => {
		expect(() =>
			...
			/Missing values: EXPO_PUBLIC_REVENUECAT_IOS_API_KEY, EXPO_PUBLIC_REVENUECAT_ANDROID_API_KEY, EXPO_PUBLIC_PRIVACY_URL, EXPO_PUBLIC_TERMS_URL, EXPO_PUBLIC_SUBSCRIPTION_TERMS_URL, EXPO_PUBLIC_CANCELLATION_URL, EXPO_PUBLIC_SUPPORT_URL, EXPO_PUBLIC_NATIVE_STORE_PURCHASES_ENABLED/,
		);
	});
	...
	it("requires only the current platform store key during an EAS build", () => {
		const sharedReleaseConfig = {
			...
			EXPO_PUBLIC_NATIVE_STORE_PURCHASES_ENABLED: "false" as const,
		};
