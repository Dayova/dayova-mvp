...
const identityOutputRules = {
	...
	state: optional(isGermanFederalState),
} as const satisfies EventPropertyRules;
...
const canonicalAnalyticsKey = (propertyName: string) =>
...
const isProhibitedAnalyticsKey = (propertyName: string) =>
	PROHIBITED_ANALYTICS_KEYS.has(canonicalAnalyticsKey(propertyName));
	...
	return {
		identify(input: AnalyticsIdentityInput | null) {
			...
			if (
				...
				(!state || !isGermanFederalState(state))
			) {
			...
			adapter.identify(distinctId, {
				...
				...(state && isGermanFederalState(state) ? { state } : {}),
			});
		},
