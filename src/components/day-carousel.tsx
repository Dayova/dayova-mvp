...
const DayCell = memo(function DayCell({
	...
	return (
		...
		>
			...
			>
				<Text
					className={`font-bold font-poppins ${isSelected ? "text-20 text-white" : "text-16 text-text/70"}`}
					style={{
						marginTop: 5,
...
export const DayCarousel = forwardRef<DayCarouselHandle, DayCarouselProps>(
	function DayCarousel(
		...
		const getItemLayout = useMemo(
			() =>
				(_: ArrayLike<DayCarouselItem> | null | undefined, index: number) => ({
					index,
		...
		const scrollToDayIndex = useCallback((index: number, animated: boolean) => {
		...
		const renderDay = ({
			item,
			index,
		}: ListRenderItemInfo<DayCarouselItem>) => (
			<DayCell
				day={item}
				index={index}
