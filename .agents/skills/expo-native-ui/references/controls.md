...
Use `$expo-ui` to choose among universal `Button`, `Switch`, `Checkbox`, `Slider`, `TextInput`, and `Picker`; community-library drop-in replacements; or platform-specific SwiftUI and Jetpack Compose trees. Do not copy stale third-party imports from this reference.
